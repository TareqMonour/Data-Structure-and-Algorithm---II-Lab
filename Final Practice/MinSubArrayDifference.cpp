...
ReturnValues minDifferenceSubArray(int A[], int start, int end) {
    if (start == end) {
        ReturnValues returnValues;
    ...
    ReturnValues left = minDifferenceSubArray(A, start, middle);
    ReturnValues right = minDifferenceSubArray(A, middle + 1, end);
...
int main() {
    ...
    ReturnValues result = minDifferenceSubArray(A, 0, n - 1);

    cout << "Minimum Subarray Difference: " << result.difference << endl;
