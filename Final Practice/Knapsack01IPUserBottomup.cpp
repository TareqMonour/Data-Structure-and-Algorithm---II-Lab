...
int main() {
    ...
    cout << "Enter the price and weight for each item:\n";
    for (int i = 1; i <= numOfItems; ++i) {
        cout << "Price for item " << i << ": ";
        cin >> P[i];
        cout << "Weight for item " << i << ": ";
        cin >> W[i];
    }
    ...
    knapsack01BottomUp(capacity, numOfItems);
    printMemo(capacity, numOfItems);
    printPrintTable(capacity, numOfItems);
    printTheItems(capacity, numOfItems);
    return 0;
}
