...
int countNonPrimeNumbers(int A[], int start, int end){
    if(start==end){
        int number=A[start];
        if(P[number]==false){
            return 1;
    ...
    int count1 = countNonPrimeNumbers(A, start,mid);
    int count2 = countNonPrimeNumbers(A, mid+1,end);
...
int main(){
    ...
    int result = countNonPrimeNumbers(A,start,end);
